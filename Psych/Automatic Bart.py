...
class BART:
    ...
    def setup_display(self):
        ...
        self.instruction_text = visual.TextStim(
            self.win,
            text='',
            pos=[0, button_y - int(120 * scale_factor)],
            color='black',
    ...
    def show_instructions(self):
        ...
        instructions = [
            "Welcome to the Automatic Balloon Analogue Risk Task (BART)\n\n" +
            ...
            "• Click PUMP to pump that many times automatically\n" +
            "• Money is collected automatically after pumping\n\n" +
    ...
    def handle_wheel_interaction(self, mouse_pos, mouse_pressed):
        ...
        if distance <= self.wheel_radius:
            if mouse_pressed and not self.wheel_dragging:
                ...
                # SMOOTHER ROTATION: Much more sensitive to mouse movement
                scaled_angle_diff = angle_diff  # Make it 2x as sensitive
    ...
    def start_pump_simulation(self):
        ...
        print(f"Starting pump simulation: {self.pumps_to_simulate} pumps")
    ...
    def update_pump_simulation(self):
        ...
        # Use PsychoPy's getTime() method instead of tick()
        ...
        if current_time - self.pump_timer >= self.pump_interval:
            ...
            if self.pumps_simulated >= self.pumps_to_simulate:
                ...
                # AUTO-COLLECT: Automatically collect money after successful pumping
                core.wait(0.5)  # Brief pause to show final balloon state
                self.collect_money()
                return False
    ...
    def record_trial_data(self, exploded):
        ...
        data_row = {
            ...
            'explosion_point': trial_info['explosion_point'],
            'pumps_selected': self.selected_pumps,
            'pumps_actual': self.current_pumps,
            'exploded': exploded,
    ...
    def update_displays(self):
        ...
        if self.is_pumping:
            self.instruction_text.text = f'Pumping... ({self.pumps_simulated}/{self.pumps_to_simulate})\nTemporary Bank: ${self.temporary_bank:.2f}'
        elif self.temporary_bank > 0:
            self.instruction_text.text = f'Temporary Bank: ${self.temporary_bank:.2f}\nCurrent Pumps: {self.current_pumps}'
        else:
            self.instruction_text.text = f'Drag wheel to select pumps, then click PUMP\nCurrent Pumps: {self.current_pumps}'
    ...
    def end_experiment(self):
        ...
        all_pumps = [trial['pumps_actual'] for trial in self.trial_data]
        mean_total_pumps = np.mean(all_pumps) if all_pumps else 0
        
        # Block analysis (trials 1-10, 11-20, 21-30)
        block1_pumps = [trial['pumps_actual'] for trial in self.trial_data[0:10]]
        block2_pumps = [trial['pumps_actual'] for trial in self.trial_data[10:20]]
        block3_pumps = [trial['pumps_actual'] for trial in self.trial_data[20:30]]
        ...
        if mean_total_pumps > 0:
            cv_total = np.std(all_pumps) / mean_total_pumps
        else:
            cv_total = 0
            
        cv_block1 = np.std(block1_pumps) / mean_block1 if mean_block1 > 0 else 0
        cv_block2 = np.std(block2_pumps) / mean_block2 if mean_block2 > 0 else 0
        cv_block3 = np.std(block3_pumps) / mean_block3 if mean_block3 > 0 else 0
        
        # Explosion analysis
        total_explosions = sum(1 for trial in self.trial_data if trial['exploded'])
        block1_explosions = sum(1 for trial in self.trial_data[0:10] if trial['exploded'])
        block2_explosions = sum(1 for trial in self.trial_data[10:20] if trial['exploded'])
        block3_explosions = sum(1 for trial in self.trial_data[20:30] if trial['exploded'])
        
        # Money earned by block
        total_money = self.total_earned
        block1_money = sum(trial['earned_this_balloon'] for trial in self.trial_data[0:10])
        block2_money = sum(trial['earned_this_balloon'] for trial in self.trial_data[10:20])
        block3_money = sum(trial['earned_this_balloon'] for trial in self.trial_data[20:30])
    ...
    def save_data(self):
        ...
        filename = f"AutoBart_data_{self.participant_id}_{self.treatment}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def save_comprehensive_summary(self):
        """Save comprehensive summary with automatic BART statistics"""
        try:
            summary_filename = f"AutoBart_summary_{self.participant_id}_{self.treatment}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            ...
            # Coefficient of Variability
            cv_total = np.std(all_pumps) / mean_total_pumps if mean_total_pumps > 0 else 0
            cv_block1 = np.std(block1_pumps) / mean_block1 if mean_block1 > 0 else 0
            cv_block2 = np.std(block2_pumps) / mean_block2 if mean_block2 > 0 else 0
            cv_block3 = np.std(block3_pumps) / mean_block3 if mean_block3 > 0 else 0
            ...
            exploded_trials = [i for i, trial in enumerate(self.trial_data) if trial['exploded']]
            post_explosion_trials = [i+1 for i in exploded_trials if i+1 < len(self.trial_data)]
            
            exploded_pumps = [self.trial_data[i]['pumps_actual'] for i in exploded_trials]
            post_explosion_pumps = [self.trial_data[i]['pumps_actual'] for i in post_explosion_trials]
            
            mean_exploded = np.mean(exploded_pumps) if exploded_pumps else 0
            mean_post_explosion = np.mean(post_explosion_pumps) if post_explosion_pumps else 0
            difference_post_explosion = mean_post_explosion - mean_exploded if exploded_pumps and post_explosion_pumps else 0
            
            with open(summary_filepath, 'w', encoding='utf-8') as f:
                f.write(f"Automatic BART Comprehensive Summary Report\n")
                f.write(f"Participant ID: {self.participant_id}\n")
                f.write(f"Treatment: {self.treatment}\n")
                f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                
                f.write(f"TASK DESIGN:\n")
                f.write(f"Total Trials: 30\n")
                ...
                f.write(f"Total Money Earned: ${total_money:.2f}\n")
                f.write(f"Total Explosions: {total_explosions}\n")
                f.write(f"Total Coefficient of Variability: {cv_total:.3f}\n\n")
                
                f.write(f"=== BLOCK ANALYSIS ===\n")
                f.write(f"Block 1 (Trials 1-10):\n")
                f.write(f"  Mean Pumps: {mean_block1:.2f}\n")
                f.write(f"  Explosions: {block1_explosions}\n")
                f.write(f"  Money Earned: ${block1_money:.2f}\n")
                f.write(f"  Coefficient of Variability: {cv_block1:.3f}\n\n")
                
                f.write(f"Block 2 (Trials 11-20):\n")
                f.write(f"  Mean Pumps: {mean_block2:.2f}\n")
                f.write(f"  Explosions: {block2_explosions}\n")
                f.write(f"  Money Earned: ${block2_money:.2f}\n")
                f.write(f"  Coefficient of Variability: {cv_block2:.3f}\n\n")
                
                f.write(f"Block 3 (Trials 21-30):\n")
                f.write(f"  Mean Pumps: {mean_block3:.2f}\n")
                f.write(f"  Explosions: {block3_explosions}\n")
                f.write(f"  Money Earned: ${block3_money:.2f}\n")
                f.write(f"  Coefficient of Variability: {cv_block3:.3f}\n\n")
                
                f.write(f"=== POST-EXPLOSION ANALYSIS ===\n")
                f.write(f"Mean Pumps on Exploded Balloons: {mean_exploded:.2f}\n")
                f.write(f"Mean Pumps on Post-Explosion Balloons: {mean_post_explosion:.2f}\n")
                f.write(f"Difference (Post - Exploded): {difference_post_explosion:.2f}\n\n")
                ...
                if mean_total_pumps < 44:
                    f.write(f"Strategy: Highly risk-averse (very conservative)\n")
                elif mean_total_pumps < 54:
                    f.write(f"Strategy: Risk-averse (conservative)\n")
                elif mean_total_pumps < 74:
                    f.write(f"Strategy: Near-optimal risk-taking\n")
                elif mean_total_pumps < 84:
                    f.write(f"Strategy: Risk-seeking (aggressive)\n")
                else:
                    f.write(f"Strategy: Highly risk-seeking (very aggressive)\n")
