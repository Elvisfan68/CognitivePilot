...
class BART:
    def __init__(self):
        ...
        # Data storage
        self.pump_data = []  # Track every individual pump
        self.trial_data = []
    ...
    def setup_display(self):
        ...
        wheel_radius = int(60 * scale_factor)
        
        # Balloon (move up more)
        self.balloon = visual.Circle(
            self.win,
            radius=50,
            pos=[0, int(150 * scale_factor)],
            fillColor='blue',
            lineColor='darkblue',
            lineWidth=2
        )
    ...
    def update_wheel_position(self):
        ...
        if self.in_topoff_mode:
            ...
            self.wheel_angle = pump_ratio * 2 * math.pi
        else:
    ...
    def handle_wheel_interaction(self, mouse_pos, mouse_pressed):
        ...
        if distance <= self.wheel_radius:
            if mouse_pressed and not self.wheel_dragging:
                ...
                if self.in_topoff_mode:
                    # TOP-OFF MODE: 1-9 pumps only
                    normalized_angle = (self.wheel_angle % (2 * math.pi))
                    pump_index = int(normalized_angle / (2 * math.pi) * 9)
                    new_pumps = max(1, min(9, pump_index + 1))
                else:
                    # NORMAL MODE: 1-128 pumps
                    total_angle_range = (127 / 30) * 2 * math.pi
    ...
    def log_pump_decision(self, pump_number_in_trial, exploded=False):
        """Log each individual pump as a decision"""
        ...
        pump_data = {
            ...
            'pump_number': pump_number_in_trial,
            'explosion_point': trial_info['explosion_point'],
            'exploded': exploded,
            'temporary_bank_after_pump': self.temporary_bank,
            'balloon_size_after_pump': self.current_balloon_size,
            'session_number': self.session_number + 1,
            'is_topoff_pump': self.current_session_is_topoff,
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
        }
        
        self.pump_data.append(pump_data)
    ...
    def update_pump_simulation(self):
        ...
        if current_time - self.pump_timer >= self.pump_interval:
            ...
            if self.current_pumps >= explosion_point:
                ...
                self.log_pump_decision(self.current_pumps, exploded=True)
                self.balloon_pop()
                return False
            ...
            # Log this pump decision
            self.log_pump_decision(self.current_pumps, exploded=False)
    ...
    def record_pump_session(self):
        ...
        if was_topoff_session:
            ...
            # First session completed - show top-off option if applicable
            if self.session_number == 1 and not self.has_topped_off:
                print("DEBUG: Showing top-off option")
    ...
    def collect_money_after_topoff(self):
        ...
        if self.temporary_bank > 0:
            ...
            print(f"DEBUG: Auto-collecting after topoff - tracking INTENDED {self.intended_pumps_total} pumps")
            
            # Animate money collection
    ...
    def collect_money(self):
        ...
        if self.temporary_bank > 0 and not self.is_pumping:
            ...
            self.last_balloon_pumps = self.intended_pumps_total
            self.last_balloon_exploded = False
            print(f"DEBUG: Collecting - tracking INTENDED {self.intended_pumps_total} pumps")
    ...
    def balloon_pop(self):
        ...
        # Record the current pump session for explosions
        ...
        self.last_balloon_earned = 0.0
        print(f"DEBUG: Exploding - tracking INTENDED {self.intended_pumps_total} pumps")
    ...
    def record_explosion_session(self):
        """Record pump session when balloon explodes"""
        trial_info = self.trial_sequence[self.current_trial]
        ...
        self.pump_sessions.append(session_data)
        print(f"EXPLOSION Session {self.session_number}: INTENDED {self.selected_pumps}, ACTUAL {self.pumps_simulated}")
    ...
    def update_displays(self):
        ...
        if self.is_pumping:
            ...
            instruction_lines = [
                ...
                f'Total: {self.current_pumps}',
                f'Temp Bank: ${self.temporary_bank:.2f}',
                'PUMP to add (auto-collects after)'
            ]
            self.instruction_text.text = '\n'.join(instruction_lines)
    ...
    def save_data(self):
        ...
        filename = f"BART_TopOff_data _{self.participant_id}_{self.treatment}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def quit_experiment(self):
        ...
        if self.trial_data or self.pump_data:
            self.save_data()
        self.win.close()
        core.quit()
